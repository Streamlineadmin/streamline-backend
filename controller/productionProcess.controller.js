...
async function updateProductionProcess(req, res) {
    try {
        ...
        const updatedProcess = await models.ProductionProcess.findOne({ where: { id } });
        res.status(200).json({ message: "Production Process updated successfully", data: updatedProcess });
    } catch (error) {
        console.error("Error:", error);
        res.status(500).json({ message: "Something went wrong!", error: error.message });
    }
...
async function deleteProductionProcess(req, res) {
    try {
        ...
        res.status(200).json({ message: "Production Process deleted successfully" });
    } catch (error) {
        console.error("Error:", error);
        res.status(500).json({ message: "Something went wrong!", error: error.message });
    }
