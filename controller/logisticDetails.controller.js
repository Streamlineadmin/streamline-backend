...
async function addLogisticDetails(req, res) {
    try {
        const {
            documentType,
            logisticType,
        ...
        if (!documentType || !logisticType || !companyId || !userId) {
            return res.status(400).json({
                message:
                    "Document Type, Logistic Type, Company ID, and User ID are required.",
            });
        }
        ...
        const existing = await models.LogisticDetails.findOne({
            where: {
        ...
        if (existing) {
            return res.status(409).json({
                message: `Logistic details already exist for document type "${existing.documentType}".`,
            });
        }

        const records = documentTypes.map((docType) => ({
            companyId,
            userId,
        ...
        const createdRecords = await models.LogisticDetails.bulkCreate(records, {
            returning: true,
        });
